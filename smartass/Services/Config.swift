...
enum Config {
    ...
    static var perplexityApiKey: String {
        ...
        if let path = Bundle.main.path(forResource: "Config", ofType: "plist"),
           let dict = NSDictionary(contentsOfFile: path) as? [String: Any],
           let apiKey = dict["PerplexityApiKey"] as? String {
            return apiKey
        }
        ...
        return "YOUR_DEVELOPMENT_KEY_HERE"
        #else
        fatalError("Perplexity API Key not found. Please set PERPLEXITY_API_KEY environment variable or add it to Config.plist")
        #endif
    }
}
